...
def dual_spls_glb(X, y, n_components, ppnu, indG, noise_level=1e-6, verbose=True):
    ...
    for k in range(n_components):
        ...
        mu = max(np.linalg.norm(Znu, 2), 1e-6)
        denominator = mu**2 + np.dot(nu, norm1Znu)
        ...
        if verbose:
            print(f"Dual PLS GLB ic={k+1}, nbzeros={np.sum(np.isclose(w,0))}, max|w|={np.max(np.abs(w))}")
